...
//        $result = substr($result, 1, -1);
    ...
    function header() {
        ...
        if (GESHI_HEADER_NONE == $this->header_type) {
            if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                return "$header<ul$attributes$ol_attributes>";
            }
        ...
        if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
            if ($this->header_type == GESHI_HEADER_PRE) {
                return "<pre$attributes>$header<ul$ol_attributes>";
            } else if ($this->header_type == GESHI_HEADER_DIV ||
                $this->header_type == GESHI_HEADER_PRE_VALID) {
                return "<div$attributes>$header<ul$ol_attributes>";
    ...
    function get_stylesheet($economy_mode = true) {
        ...
        // Set the <ul> to have no effect at all if there are line numbers
        // (<ul>s have margins that should be destroyed so all layout is
